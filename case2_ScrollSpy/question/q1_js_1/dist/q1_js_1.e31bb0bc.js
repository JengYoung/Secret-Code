...
window.addEventListener("scroll", e => {
  const {
    scrollTop
  } = e.target.scrollingElement; // do something

  const targetIndex = offsetTops.findIndex(_ref => {
    let [from, to] = _ref;
    return scrollTop >= from && scrollTop < to;
  });
  Array.from(navElem.children).forEach((c, i) => {
    if (i !== targetIndex) c.classList.remove('on');else c.classList.add('on');
  });
}); // 이동
...
//# sourceMappingURL=/q1_js_1.e31bb0bc.js.map